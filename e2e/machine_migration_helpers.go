...
func verifyMapiAutoritative(cl client.Client) *mapiv1beta1.Machine {
	ctx := context.Background()
	machineList := &mapiv1beta1.MachineList{}
	By(fmt.Sprintf("Listing worker machines in namespace: %s", capiframework.CAPINamespace))
	workerLabelSelector := client.MatchingLabels{RoleLabel: "worker"}
	Eventually(cl.List(ctx, machineList, client.InNamespace(capiframework.CAPINamespace), workerLabelSelector)).Should(Succeed(), "Failed to list worker machines")
	Expect(machineList.Items).NotTo(BeEmpty(), "No worker machines found in the namespace to use as a template")

	var templateMachine *mapiv1beta1.Machine
...
func verifyMapiMachineSynchronizedPaused(machine *mapiv1beta1.Machine) {
	Expect(machine.Status.Conditions).To(ContainElement(
		SatisfyAll(
			HaveField("Type", SynchronizedCondition),
	...
	Expect(machine.Status.Conditions).To(ContainElement(
		SatisfyAll(
			HaveField("Type", MAPIPausedCondition),
...
func verifyCapiMachinePaused(cl client.Client, machineName string) {
	...
	targetMachine, err := capiframework.GetMachine(cl, machineName, capiframework.CAPINamespace)
