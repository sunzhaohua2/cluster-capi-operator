...
func verifyMachineSetMAPIPausedCondition(mapiMachineSet *machinev1beta1.MachineSet, authority machinev1beta1.MachineAuthority) {
...
func verifyMachineSetCAPIPausedCondition(capiMachineSet *clusterv1.MachineSet, authority machinev1beta1.MachineAuthority) {
	var conditionMatcher types.GomegaMatcher
...
func waitForCAPIMachineSetMirror(cl client.Client, machineName string) *clusterv1.MachineSet {
	By(fmt.Sprintf("Verifying there is a CAPI MachineSer mirror for  MAPI MachineSet %s", machineName))
	var capiMachineSet *clusterv1.MachineSet
	Eventually(func() error {
...
func getAWSProviderSpecFromMachineSet(machineSetName string) (*machinev1beta1.AWSMachineProviderConfig, error) {
	currentMachineSet, _ := mapiframework.GetMachineSet(ctx, cl, machineSetName)
	if currentMachineSet.Spec.Template.Spec.ProviderSpec.Value == nil {
	...
	providerSpec, err := mapi2capi.AWSProviderSpecFromRawExtension(currentMachineSet.Spec.Template.Spec.ProviderSpec.Value)
	if err != nil {
...
func getMAPIMachineSetInstanceType(mapiMachineSet *machinev1beta1.MachineSet) string {
	providerSpec, err := getAWSProviderSpecFromMachineSet(mapiMachineSet.Name)
	Expect(err).ToNot(HaveOccurred(), "failed to get AWS provider spec from MachineSet")

	return providerSpec.InstanceType
}

func verifyMAPIMachineSetInstanceType(mapiMachineSet *machinev1beta1.MachineSet, expectedInstanceType string) {
	By(fmt.Sprintf("Verifying MAPI MachineSet %s has instanceType = %s", mapiMachineSet.Name, expectedInstanceType))

	Eventually(func() string {
		return getMAPIMachineSetInstanceType(mapiMachineSet)
